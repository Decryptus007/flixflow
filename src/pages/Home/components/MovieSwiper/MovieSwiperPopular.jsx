...
const MovieSwiperPopular = forwardRef((props, ref) => {
  const [mostPopularMovies, setMostPopularMovies] = useState([])

  //It will check if the data exist in localstorage before fetching new contents
  function getMostPopularOptimized() {
    if (localStorage.getItem('most-popular') !== null) {
      setMostPopularMovies(JSON.parse(localStorage.getItem('most-popular')))
    } else {
      getMostPopular()
    }
  }
  ...
  async function getMostPopular() {
    try {
      ...
      localStorage.getItem('most-popular') !== null && localStorage.removeItem('most-popular')
      const response = await axios.get(`${import.meta.env.VITE_BASE_URL}/MostPopularMovies/${import.meta.env.VITE_IMDB_API_KEY}`);
  ...
  useEffect(() => {
    ...
    mostPopularMovies.length && localStorage.setItem('most-popular', JSON.stringify(mostPopularMovies))
...
export default MovieSwiperPopular
